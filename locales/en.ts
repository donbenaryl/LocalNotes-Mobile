const en = {
  common: {
    goBack: 'Go back',
    continue: 'Continue',
    cancel: 'Cancel',
    close: 'Close',
    done: 'Done',
    more: 'More options',
    user: 'User',
    apply: 'Apply',
    search: 'Search',
    save: 'Save',
    pleaseWait: 'Please wait…',
    sending: 'Sending…',
    retry: 'Try again',
    orSignInWith: 'OR CONTINUE WITH',
    passwordHint:
      'Must be at least 8 characters, with uppercase and lowercase letters, and one of ! / @ .',
    optional: '(optional)',
    scrollToTop: 'Scroll to top',
    createPick: 'Create a Pick',
    createList: 'Create a List',
  },
  imageUpload: {
    addPhoto: 'Add Photo',
    chooseFromLibrary: 'Choose from library',
    takePhoto: 'Take photo',
    count: '{{current}}/{{max}}',
  },
  validation: {
    emailRequired: 'Email is required',
    emailInvalid: 'Enter a valid email address',
    passwordRequired: 'Password is required',
    passwordTooShort: 'Password must be at least 8 characters',
    passwordWeakFormat: 'Password must contain both letters and numbers',
    passwordNeedsMixedCase:
      'Password must include both uppercase and lowercase letters',
    passwordNeedsSpecialChar: 'Password must include one of ! / @ .',
    passwordTooCommon: 'This password is too common.',
    confirmPasswordRequired: 'Please confirm your password',
    passwordsMismatch: 'Passwords do not match',
    fullNameRequired: 'Full name is required',
    displayNameRequired: 'Display name is required',
    nameTooLong: 'Must be 250 characters or fewer',
    contactNameRequired: 'Contact name is required',
    dateOfBirthRequired: 'Date of birth is required',
    dateOfBirthPast: 'Date of birth must be in the past',
    businessNameRequired: 'Business name is required',
    businessWebsiteRequired: 'Business website is required',
    businessWebsiteInvalid: 'Enter a valid URL (e.g. https://example.com)',
    workEmailRequired: 'Business email is required',
    usernameRequired: 'Username is required',
    usernameTooShort: 'Username must be at least 3 characters.',
    usernameTooLong: 'Username must be at most 30 characters.',
    usernameEmailTooLong: 'Email usernames must be at most 150 characters.',
    usernameInvalidFormat:
      'Enter a valid email address, or a username with only letters, numbers, underscores, and periods.',
    usernameAvailable: 'Username is available.',
    usernameTaken: 'This username is already taken.',
    usernameChecking: 'Checking username…',
    usernameCurrent: 'This is your current username.',
    usernameVerifyFailed: 'Could not verify username. Please try again.',
    usernameUnavailable: 'Please choose a valid, available username.',
  },
  alerts: {
    signInFailed: 'Sign In Failed',
    signUpFailed: 'Sign Up Failed',
    resetFailed: 'Reset Failed',
    resendFailed: 'Resend Failed',
    verificationFailed: 'Verification Failed',
    comingSoon: 'Coming Soon',
    comingSoonMessage: 'This feature will be available soon.',
    codeSent: 'Code Sent',
    codeSentMessage: 'A new code has been sent to your email.',
    qrScan: 'QR Scan',
    qrScanMessage: 'QR code scanning will be available soon.',
    error: 'Error',
  },
  onboarding: {
    tagline: 'Your People, Your Taste',
    subtitle: 'Skip the ratings, Discover what’s aligned with you',
    signIn: 'Sign In',
    getStarted: 'Get Started',
  },
  auth: {
    signIn: {
      title: 'Discover spots that',
      subtitle: 'match your taste.',
      description: "Built around personality, not stars. Sign in or create your account.",
      logoDescription: 'find places by people who get you.',
      emailLabel: 'Email',
      emailPlaceholder: 'Enter your email',
      passwordLabel: 'Password',
      passwordPlaceholder: 'Your password',
      forgotPassword: 'Forgot your password?',
      signingIn: 'Signing in…',
      button: 'Continue',
      noAccount: "New to LocalNotes?",
      createAccount: 'Create Account',
      terms: {
        prefix: 'By continuing with Apple or Google, you agree to our ',
        termsLink: 'Terms',
        separator: ', ',
        privacyLink: 'Privacy Policy',
        andSeparator: ', and ',
        guidelinesLink: 'Community Guidelines',
      },
    },
    consent: {
      badge: 'Required before you continue',
      prefix: 'I agree to the ',
      terms: 'Terms',
      separator: ', ',
      privacy: 'Privacy Policy',
      andSeparator: ', and ',
      guidelines: 'Community Guidelines',
      suffix: '.',
      zeroTolerance:
        'LocalNotes has zero tolerance for objectionable content and abusive users.',
      error: 'Please agree to continue',
    },
    signUp: {
      title: 'Sign Up',
      headerTitle: 'Create account',
      kicker: 'Get started',
      // welcomeTitle: 'Create your account.',
      // welcomeDescription:
      //   'Recommendations from people who share your taste. No stars, no strangers.',
      welcomeTitle: "Welcome — let's get you",
      welcomeHighlight: 'set up.',
      welcomeDescription:
        'Email path shown. Apple/Google skip the password fields but see the same consent gate below.',
      signUpSuccess: 'Sign up successful! Please verify your email.',
      emailVerified: 'Email verified successfully.',
      invalidOtp: 'Invalid verification code. Please try again.',
      resendSuccess: 'Verification code resent successfully.',
      checkInboxTitle: 'Check your inbox.',
      checkInboxDescription: 'We sent a 6-digit code to',
      checkInboxSuffix: 'Enter it below to continue.',
      changeEmail: 'Use a different email',
      resending: 'Resending…',
      resendCode: 'Resend code',
      fullNameLabel: 'Full Name',
      fullNamePlaceholder: 'e.g. Jane Doe',
      displayNameLabel: 'Display Name',
      displayNamePlaceholder: 'e.g. Jane Doe',
      firstNameLabel: 'First Name',
      firstNamePlaceholder: 'First',
      lastNameLabel: 'Last Name',
      lastNamePlaceholder: 'Last',
      nameLabel: 'Your Name',
      namePlaceholder: 'What should we call you?',
      emailLabel: 'Email',
      emailPlaceholder: 'you@example.com',
      usernameLabel: 'Username',
      usernamePlaceholder: 'e.g. jane_doe',
      passwordLabel: 'Create Password',
      passwordPlaceholder: 'Enter your password',
      passwordFieldLabel: 'Password',
      passwordHelper: 'email signup only',
      confirmPasswordLabel: 'Confirm Password',
      confirmPasswordPlaceholder: 'Re-enter your password',
      requirements: {
        minLength: '8+ characters',
        number: 'A number',
        symbol: 'A symbol',
        mixedCase: 'Mixed case',
      },
      consent: {
        badge: 'Required before you continue',
        prefix: 'I agree to the ',
        terms: 'Terms',
        separator: ', ',
        privacy: 'Privacy Policy',
        andSeparator: ', and ',
        guidelines: 'Community Guidelines',
        suffix: '.',
        error: 'Please agree to continue',
      },
    },
    signUpBusiness: {
      businessNameLabel: 'Business Name',
      businessNamePlaceholder: 'e.g. Acme Corp',
      businessWebsiteLabel: 'Business Website',
      businessWebsitePlaceholder: 'https://example.com',
      workEmailLabel: 'Business Email',
      workEmailPlaceholder: 'you@example.com',
      workEmailHint:
        'Contact email for this business. Personal addresses like Gmail are allowed.',
      workEmailInvalid: 'Please provide a work email address.',
    },
    signUpFooter: {
      haveAccount: 'Have an account?',
      signIn: 'Sign In',
    },
    verifyOtp: {
      title: 'Enter OTP code',
      subtitle: 'Enter the 6-digit code we sent to your email',
      verifying: 'Verifying…',
      verifyCode: 'Verify Code',
      noCode: "Didn’t receive the code? ",
      resend: 'Click to resend',
    },
    forgotPassword: {
      title: 'Forgot Password',
      emailLabel: 'Email',
      emailPlaceholder: 'you@example.com',
      send: 'Send',
    },
    createNewPassword: {
      title: 'Create New Password',
      passwordLabel: 'Create Password',
      passwordPlaceholder: 'Enter your new password',
      confirmPasswordLabel: 'Confirm Password',
      confirmPasswordPlaceholder: 'Re-enter your new password',
      resetting: 'Resetting…',
      resetPassword: 'Reset Password',
    },
    passwordUpdated: {
      title: 'Password Updated!!',
      subtitle:
        'Your password has been successfully reset. You can now sign in with your new password.',
      signIn: 'Sign In',
    },
    onboarding: {
      accountTypeKicker: 'Account type',
      accountTypeTitle: 'How will you use LocalNotes?',
      accountTypeSubtitle: 'Choose the account type that fits you best.',
      individualTitle: 'Individual',
      individualDesc: 'Personal lists and recommendations',
      businessTitle: 'Business',
      businessDesc: 'Curated lists for your brand or venue',
      back: 'Back',
      detailsKicker: 'Your details',
      individualDetailsTitle: 'Tell us about yourself.',
      businessDetailsTitle: 'Tell us about your business.',
      contactNameLabel: 'Your Name',
      contactNamePlaceholder: 'Your full name',
      dateOfBirthLabel: 'Date of Birth',
      dateOfBirthPlaceholder: 'Select your date of birth',
      businessDetailsSection: 'Business Details',
      securitySection: 'Security',
      passwordPlaceholder: 'At least 8 characters',
      continueBusiness: 'Continue',
      saveFailed: 'Failed to save onboarding details.',
      saveSuccess: 'Onboarding details saved.',
    },
  },
  categoryModal: {
    title: 'Select your preferred category?',
    individualTitle: 'As an individual',
    individualDesc: 'I want to create, share, and connect.',
    businessTitle: 'As a business',
    businessDesc: 'I want to post, track, and grow.',
  },
  faceSignIn: {
    label: 'Sign in with Face ID',
    prompt: 'Unlock LocalNotes',
    unlocking: 'Unlocking…',
    failed: 'Biometric unlock was cancelled or failed.',
    sessionExpired: 'Your session expired. Please sign in again.',
  },
  personality: {
    header: 'Personality',
    skip: 'Skip',
    seeResult: 'See result',
    answered: 'answered',
    analyzing: 'Analyzing your results...',
    yourPersonality: 'Your Personality',
    tapToFlip: 'Tap to flip',
    retake: 'Retake Quiz',
    complete: 'Complete',
    sections: {
      vibeAtmosphere: 'Vibe & Atmosphere',
      styleApproach: 'Style & Approach',
      tasteDiscovery: 'Taste & Discovery',
      environmentMovement: 'Environment & Movement',
      energyPlanning: 'Energy & Planning',
    },
  },
  settings: {
    title: 'Settings',
    appearance: 'Appearance',
    darkMode: 'Dark Mode',
    lightMode: 'Light Mode',
    recentActivityFeed: 'Recent Activity Feed',
    accountSettings: 'Account Settings',
    retakePersonalityTest: 'Retake Personality Test',
    logout: 'Log Out',
    language: 'Language',
    currentLanguage: 'App Language',
    english: 'English',
  },
  accountSettings: {
    title: 'Settings',
    edit: 'Edit',
    sections: {
      account: 'Account',
      privacyVisibility: 'Privacy & visibility',
      preferences: 'Preferences',
      connected: 'Connected',
      helpLegal: 'Help & legal',
      dangerZone: 'Danger zone',
    },
    menu: {
      editProfile: 'Edit profile',
      editProfileSub: 'Name, bio, home city, photo',
      personality: 'Personality',
      personalitySub: 'Retake the quiz, see your blend',
      homeCity: 'Home city',
      homeCitySub: 'Used for default location everywhere',
      privacyVisibility: 'Privacy & Visibility',
      privacyVisibilitySub: 'Who sees your lists, discovery, blocked users',
      notifications: 'Notifications',
      language: 'Language',
      appearance: 'Appearance',
      appearanceSystem: 'System',
      appearanceLight: 'Light',
      appearanceDark: 'Dark',
      biometricUnlock: 'Unlock with Face ID',
      biometricUnlockSub: 'Require Face ID or fingerprint when opening the app',
      biometricUnavailable: 'Biometrics are not available on this device.',
      biometricEnableFailed: 'Could not enable biometric unlock.',
      connectedAccounts: 'Connected accounts',
      claimBusiness: 'Claim a business',
      claimBusinessSub: 'Manage a business listing',
      convertToBusiness: 'Convert to business account',
      convertToBusinessSub: 'Add business details and switch account type',
      addAnotherBusiness: 'Add another business',
      addAnotherBusinessSub: 'Create another business you own',
      termsPrivacy: 'Terms & privacy',
      support: 'Support',
      supportSub: 'Phone, email, and mailing address',
      about: 'About LocalNotes',
      signOut: 'Sign out',
      deleteAccount: 'Delete account',
      deleteAccountSub: 'Permanently remove your account and all data',
      deleteAccountTitle: 'Delete account?',
      deleteAccountMessage:
        'This will permanently remove your account and all data. This action cannot be undone.',
      deleteAccountTypeLabel: 'Type "DELETE" to confirm',
      deleteAccountConfirm: 'Delete',
      deleteAccountCancel: 'Cancel',
      comingSoon: 'Coming soon',
    },
    notifications: {
      title: 'Notifications',
      intro:
        'Choose what reaches you. Push goes to your phone; email to your inbox.',
      sections: {
        activity: 'Activity',
        fromFollows: 'From people you follow',
        businessOffers: 'Business & offers',
        spotlight: 'Spotlight',
        emailDigests: 'Email digests',
        quietHours: 'Quiet hours',
      },
      likesAndSaves: 'Likes & saves',
      likesAndSavesSub: 'When someone saves your list or pick',
      newFollowers: 'New followers',
      mentions: 'Mentions',
      mentionsSub: 'When someone @-tags you',
      commentsAndReplies: 'Comments & replies',
      newLists: 'New lists',
      newPicks: 'New picks',
      newPicksSub: 'Quieter — only from your top 20 follows',
      businessOffers: 'Offers from businesses you follow',
      featuredOffers: 'Featured offers near you',
      featuredOffersSub: 'Paid placements; max one push per week',
      spotlightDigest: 'Weekly Spotlight digest',
      spotlightDigestSub: "This week's Spotlight edition for your city",
      weeklyRecap: 'Weekly recap',
      weeklyRecapSub: "What's new from your circle, every Sunday",
      reengagementNudges: 'Re-engagement nudges',
      reengagementNudgesSub: '"You captured 4 places — make a list?"',
      quietHours: 'Pause notifications',
      quietHoursSub: 'No push between 10pm and 8am',
    },
    privacy: {
      title: 'Privacy & visibility',
      loadError: 'Could not load your latest privacy settings. Showing cached values.',
      sections: {
        profile: 'Profile',
        discovery: 'Discovery',
        location: 'Location',
        blocked: 'Blocked & restricted',
        yourData: 'Your data',
      },
      showHomeCity: 'Show home city on profile',
      showHomeCitySub: '"{{city}}" appears below your name',
      showHomeCitySubFallback: 'Your city appears below your name',
      showMatchPercentSub: '"96% match with you" on your profile',
      showPersonalityName: 'Show personality archetype name',
      showPersonalityNameSub: '"{{name}}" label on your profile',
      showPersonalityNameSubFallback: 'Your personality label on your profile',
      appearInSearch: 'Appear in Search',
      appearInSearchSub: 'Off means people can only find you by direct link',
      showInSmartPicks: 'Show in SmartPicks recommendations',
      showInSmartPicksSub:
        'Your lists may surface to people with matching taste',
      allowMentions: 'Allow @-mentions from anyone',
      allowMentionsSub: 'Off means only people you follow can mention you',
      showSavedList: 'Show saved list',
      showSavedListSub: 'Off hides your saved list from other users',
      showLikesAndComments: "Show likes and comments on others' lists",
      showLikesAndCommentsSub:
        'Off hides that you liked or commented from other people',
      usePreciseLocation: 'Use precise location',
      usePreciseLocationSub: 'Off uses approximate location instead',
      blockedUsers: 'Blocked users',
      blockedUsersSub: "They can't follow you, see your lists, or mention you",
      blockedUsersTitle: 'Blocked users',
      blockedUsersEmpty: 'You haven’t blocked anyone.',
      blockedUsersLoadError: 'Couldn’t load blocked users.',
      unblock: 'Unblock',
      unblockSuccess: 'Unblocked.',
      unblockError: 'Couldn’t unblock this user. Try again.',
      hiddenLists: 'Hidden lists',
      hiddenListsSub: "Lists you've muted from your feed",
      downloadData: 'Download your data',
      downloadDataSub: "Get a copy of everything you've made and saved",
    },
    connectedAccounts: {
      title: 'Connected accounts',
      intro:
        "Connect your review accounts to mirror reviews you've already written onto your LocalNotes profile. Disconnect any time — your reviews disappear within 24 hours.",
      connectedSection: 'Connected · {{count}}',
      availableSection: 'Available',
      connect: 'Connect',
      disconnect: 'Disconnect',
      reviewsSynced: '{{count}} reviews · last synced {{time}}',
      amazonHint: 'Mirror your product reviews',
      tripadvisorHint: 'Mirror your hotel and restaurant reviews',
      googleHint: 'Mirror your Google reviews',
      yelpHint: 'Mirror your Yelp reviews',
      howItWorksTitle: 'How this works.',
      howItWorksBody:
        'Google uses OAuth (Data Portability) to copy your Maps reviews — we never see your password. Yelp, Amazon, and TripAdvisor are coming soon. We show reviews on your profile with a source badge. We do not write reviews for you or compute a LocalNotes rating.',
      comingSoon: 'Coming soon',
      connectError: 'Could not connect Google. Try again.',
      disconnectError: 'Could not disconnect. Try again.',
      connectCancelled: 'Connection cancelled.',
    },
  },
  profile: {
    info: {
      homeAddress: 'Home address',
      editProfile: 'Edit profile',
      shareProfile: 'Share this profile',
      editBusiness: 'Edit business',
      shareBusiness: 'Share this business',
      joined: 'Joined {{year}}',
      locationJoined: '{{location}} · Joined {{year}}',
      bioMore: 'more',
      bioLess: 'less',
      tasteMatchBlurb: 'you both lean quiet, cozy & caffeine-first',
      loadError: 'Failed to load profile.',
      businessLoadError: 'Failed to load business.',
      stats: {
        lists: 'Lists',
        following: 'Following',
        followers: 'Followers',
        saves: 'Saves',
        shares: 'Shares',
      },
      businessHome: {
        title: 'Business Home',
        subtitle: 'Insights · Campaigns · Alerts',
        comingSoonTitle: 'Business Home',
        comingSoonMessage: 'Business Home is coming soon.',
      },
      featuredIn: {
        eyebrow: 'Featured in',
        lists: 'lists',
        monthDelta: '+{{count}} this month.',
        typesPrefix: 'Most curators saving you are ',
        typesAnd: ' and ',
        typesSuffix: ' types.',
        typePrefix: 'Most curators saving you are ',
        typeSuffix: ' types.',
      },
    },
    vitalBar: {
      match: '{{percent}}% match',
    },
    tabs: {
      myLists: 'My Lists',
      picks: 'Picks',
      reviews: 'Reviews',
      saved: 'Saved Lists',
      offers: 'Offers',
      contributed: 'Contributed Lists',
      sharedWithMe: 'Shared With Me',
      about: 'About',
    },
    reviews: {
      emptyTitle: 'Bring your reviews here',
      emptyDescription:
        "Connect Google to mirror reviews you've already written. Manage connections in Settings — we never ask you to write a third review.",
      connectCta: 'Connect accounts',
      manageAccounts: 'Manage',
      syncing: 'Syncing reviews from Google…',
      noReviewsTitle: 'No reviews yet',
      noReviewsOwnDescription:
        'Your connected accounts have no reviews to show yet. Pull to refresh after writing one on Google.',
      noReviewsOtherDescription: 'This person has not shared any reviews yet.',
      helpfulCount: '{{count}} found this helpful',
      editOn: 'Edit on {{provider}} →',
    },
    about: {
      loading: 'Loading business details…',
      empty: 'Business details are not available yet.',
      contact: 'Contact',
      locations: 'Locations',
      location: 'Location',
      directionsWithDistance: 'Directions · {{distance}} mi',
      email: 'Email',
      phone: 'Phone',
      website: 'Website',
      shares: 'Shares',
    },
    filters: {
      filterBy: 'Filter by:',
      category: 'Category',
      status: 'Status',
      favorites: 'Favorites',
      all: 'All',
    },
    businessAuthorship: {
      about: 'About {{name}}',
      by: 'By {{name}}',
      picksEmptyAbout: 'No community picks about {{name}} yet',
      listsEmptyAbout: 'No lists featuring {{name}} yet',
    },
    lists: {
      loading: 'Loading lists…',
      error: 'Error loading lists',
      retry: 'Retry',
      emptyTitle: 'No lists found',
      emptyDescription: 'Create your first list to get started',
      emptyCategory: 'No {{category}} available',
      deleteTitle: 'Delete List',
      deleteMessage: 'Are you sure you want to delete this list? This action cannot be undone.',
      edit: 'Edit',
      delete: 'Delete',
      pin: 'Pin',
      follow: '+ Follow',
      following: 'Following',
      optionsTitle: 'List options',
      morePicks: '+{{count}} more picks',
      showLess: 'Show less',
      expandList: 'Open list: {{title}}, {{count}} places',
      collapseList: 'Collapse {{title}}',
      placesMeta: '{{count}} places · {{where}}',
    },
    safety: {
      shareProfile: 'Share this profile',
      report: 'Report',
      reportTitle: 'Report {{name}}?',
      reportBody: 'Reports are private. LocalNotes reviews them and may take action on accounts that break our guidelines.',
      reportDetailsPlaceholder: 'Tell us what happened',
      reportDetailsLabel: 'Add details',
      reportDetailsOptional: 'Optional',
      reportDetailsRequired: 'Required',
      reportSubmit: 'Submit report',
      reportCancel: 'Cancel',
      reportSuccess: 'Report submitted. This content is hidden from your feed.',
      reportError: 'Couldn’t submit this report. Try again.',
      reportReasons: {
        spam: 'Spam, scams, or fraud',
        harassment: 'Harassment or threats',
        hate_speech: 'Hate speech or discrimination',
        sexual_content: 'Pornographic or sexual content',
        dangerous: 'Dangerous content',
        minor_sexual_content: 'Sexual content involving a minor',
        impersonation: 'Impersonation or fake account',
        other: 'Something else',
      },
      block: 'Block {{name}}',
      blockTitle: 'Block {{name}}?',
      blockBodyBefore: 'Their content is removed from your feed immediately, and LocalNotes is notified. They won’t be able to follow you, see your lists, mention you, or reach you — and ',
      blockBodyEmphasis: 'you won’t see them either.',
      blockBodyAfter:
        ' They won’t be told. You can undo this in Settings → Privacy.',
      blockConfirm: 'Block',
      blockCancel: 'Cancel',
      blockSuccess: 'Blocked. Their content is hidden from your feed and LocalNotes was notified.',
      blockError: 'Couldn’t block this user. Try again.',
    },
    picks: {
      directions: 'Directions',
      save: 'Save',
      saved: 'Saved',
      appearsIn_one: 'Appears in {{count}} list',
      appearsIn_other: 'Appears in {{count}} lists',
      byAuthor: 'by {{name}}',
      photoCount: '{{current}}/{{total}}',
      closeDetails: 'Close pick details',
      viewPhoto: 'View photo {{current}} of {{total}} full screen',
      addPick: "Add Pick",
      picksCount: '{{count}} picks',
      loading: 'Loading picks…',
      emptyTitle: 'No picks yet',
      emptyDescription: 'Add your first pick to get started',
      savePick: 'Save Pick',
      editPick: 'Edit Pick',
      addPhoto: 'Add Photo',
      chooseFromLibrary: 'Choose from library',
      takePhoto: 'Take photo',
      photos: 'Photos',
      vibeTags: 'Vibe tags',
      vibeTagsHint: 'Optional · 1-3 descriptors',
      addAnotherTag: 'Add another…',
      tagPlaceholder: 'Type a vibe and press Enter',
      nameLabel: 'Name of the spot, book, song, etc.',
      namePlaceholder: 'Name of the spot, book, song, etc.',
      descriptionLabel: 'Short description',
      descriptionPlaceholder: "e.g. 'Strong wifi, strong espresso, opens early.'",
      locationOptional: 'Location (optional)',
      businessLocationLabel: 'Location',
      selectBranch: 'Select a branch',
      branchRequired: 'Please select a branch',
      words: 'words',
      businessRequired: 'Business name is required',
      categoryLabel: 'Category',
      categoryRequired: 'Please select at least one category',
      othersCategoryLabel: 'Specify other category',
      othersCategoryPlaceholder: 'e.g. Hidden gems',
      othersNameRequired: "Please specify the 'Others' category name",
      noContactInfo: 'No contact info',
      verifiedBusiness: 'Verified business: {{name}}',
      deletePhotoTitle: 'Delete photo',
      deletePhotoMessage: 'Are you sure you want to delete this photo from the pick?',
      deletePhotoError: 'Could not remove image',
      createError: 'Failed to create pick',
      updateError: 'Failed to update pick',
      deleteError: 'Failed to delete pick',
      deleteInUse: 'Cannot delete — pick is used in: {{lists}}',
      favoriteError: 'Failed to update favorite',
      deletePickTitle: 'Delete Pick',
      deletePickMessage: 'Are you sure you want to delete this pick? This action cannot be undone.',
      noImage: 'No Image',
      untitled: 'Untitled pick',
      verified: 'Verified business',
      pickDetails: 'Pick details',
      list: 'list',
      lists: 'lists',
    },
    activity: {
      followingLists: 'Following Lists',
      noFollowingLists: 'No following lists',
      noFollowingListsDescription: 'Follow users to see their lists here',
      activityFeed: 'Activity Feed',
      emptyFeed: 'Empty Activity Feed',
      emptyFeedDescription: 'Interact with other users and lists to get started',
      similarUsers: 'Similar Users You May like',
      similarLists: 'Similar Lists You May like',
      smartPick: 'Smart Pick',
    },
  },
  home: {
    following: {
      notesToday: 'Notes Today',
      latestActivity: 'Latest Activity',
      noActivity: 'No activity yet',
      noActivityDescription: 'Follow users to see their activity here',
      activity: {
        create: 'just published',
        update: 'just updated',
        like: 'just liked',
        save: 'just saved',
        share: 'just shared',
        comment: 'just commented on',
        follow: 'just followed',
        listObject: 'a list',
      },
    },
    newNearYou: 'New Near you',
    discover: 'Discover',
    newBadge: 'NEW · {{time}}',
    authorMeta: '{{location}} · {{count}} picks',
    unknownLocation: 'Unknown',
    location: {
      all: 'All',
      allDescription: 'Show lists everywhere',
      chooseLocation: 'Choose location',
    },
    picksCount: '{{count}} picks',
    updated: 'Updated {{time}}',
    savesCount: '↑ {{count}} saves',
    savesCountShort: '{{count}}',
    reactionsCountShort: '{{count}}',
    commentsCountShort: '{{count}}',
    morePicksBadge: '+ {{count}}',
    seeMorePicks: 'See {{count}} more picks',
    showLessPicks: 'Show less',
    saveList: 'Save list',
    follow: '+ Follow',
    loading: 'Loading lists...',
    error: 'Could not load lists',
    retry: 'Try again',
    emptyDiscover: 'No lists to show yet',
    emptyDiscoverDescription: 'Check back soon for new lists from curators near you',
    contentType: {
      lists: 'List',
      picks: 'Pick',
    },
    emptyPicksDiscover: 'No picks to show yet',
    emptyPicksDiscoverDescription: 'Check back soon for new picks from curators near you',
    filters: {
      category: 'Category',
      personalityMatch: 'Match',
      vibe: 'Vibe',
      distance: 'Distance',
      newest: 'Newest',
    },
    timeOfDay: {
      morning: 'morning',
      afternoon: 'afternoon',
      evening: 'evening',
      night: 'night',
    },
    editorial: {
      timePhrase: {
        morning: 'This morning',
        afternoon: 'This afternoon',
        evening: 'Tonight',
        night: 'Tonight',
      },
      title: '{{timePhrase}} in {{city}},',
      titleAll: '{{timePhrase}},',
      subtitle: 'picked for your taste.',
    },
    forYou: {
      eyebrow: 'For you',
      match: '{{percent}}%',
      tasteMatch: '{{percent}}%',
      authorMeta: '{{location}} · {{count}} picks',
      savedBy: 'Saved by {{count}} people like you',
      moreTags: '+{{count}} more',
    },
  },
  saved: {
    tabs: {
      drafts: 'Drafts',
      saved: 'Saved',
      sharedWithMe: 'Shared with me',
    },
    drafts: {
      listDraftsHeading: 'List drafts',
      pickCapturesHeading: 'Pick captures · today',
      emptyTitle: 'No drafts yet',
      emptyDescription: "Lists and picks you haven't published yet will show up here",
      smartSuggestEyebrow: 'Suggested',
      smartSuggestTitle_one: 'You captured {{count}} place in {{location}}.',
      smartSuggestTitle_other: 'You captured {{count}} places in {{location}}.',
      smartSuggestDetail: "Make them a list? We'll pre-fill the picks.",
      notNow: 'Not now',
      makeList: 'Make list →',
      profileLinkText: 'Looking for your published lists? They live on your profile.',
    },
    saved: {
      listsLabel: 'Lists',
      picksLabel: 'Picks',
      emptyListsTitle: 'No saved lists yet',
      emptyListsDescription: 'Lists you bookmark from other curators will show up here',
      emptyPicksTitle: 'No saved picks yet',
      emptyPicksDescription: 'Picks you bookmark from other people will show up here',
    },
    sharedWithMe: {
      collaborative: 'Collaborative',
      sharedWithMe: 'Shared with me',
      emptyTitle: 'Nothing shared yet',
      emptyDescription: 'Collaborative lists and lists shared with you will show up here',
    },
  },
  search: {
    tabs: {
      picks: 'Picks',
      lists: 'Lists',
      places: 'Places',
      people: 'People',
    },
    placeholder: 'Try “cozy date night” or a place name…',
    cancel: 'Cancel',
    clear: 'Clear',
    loading: 'Searching…',
    error: 'Could not load search results',
    retry: 'Try again',
    resultsMeta: {
      picks: '{{count}} picks',
      lists: '{{count}} lists',
      places: '{{count}} places',
      people: '{{count}} people',
      inLocation: '{{count}} in {{location}}',
    },
    empty: {
      picks: 'No picks found',
      lists: 'No lists found',
      places: 'No places found',
      people: 'No people found',
      description: "Try describing what you're looking for, or adjust your filters",
    },
    filters: {
      match: 'Match',
      matchValue: 'Match: {{percent}}%+',
      vibe: 'Vibe',
      vibeValue: 'Vibe: {{labels}}',
      nearMe: 'Near me',
    },
    business: {
      listsCount: '{{count}} lists',
      moreBranches: '+{{count}} more',
      follow: '+ Follow',
      following: 'Following',
    },
    people: {
      listsCount: '{{count}} lists',
      followersCount: '{{count}} followers',
      stats: '{{lists}} · {{followers}}',
    },
    map: {
      areaLabel: '{{location}}',
      noPins: 'No locations to show on the map',
      pinSheet: {
        lists_one: '{{count}} list',
        lists_other: '{{count}} lists',
        picks_one: '{{count}} pick',
        picks_other: '{{count}} picks',
        people_one: '{{count}} person',
        people_other: '{{count}} people',
        places_one: '{{count}} place',
        places_other: '{{count}} places',
      },
    },
  },
  offers: {
    loading: 'Loading offers...',
    error: 'Could not load offers',
    retry: 'Try again',
    empty: 'No offers yet',
    emptyDescription: 'There are no offers to show right now. Check back soon.',
    title: 'Offers',
    description: 'Deals, events, and exclusives from businesses and creators you care about',
    detail: {
      title: 'Offer',
      loading: 'Loading offer…',
      error: 'Could not load this offer',
      posted: 'Posted {{time}}',
      closeDetails: 'Close offer details',
      viewMedia: 'View offer media full screen',
      redeem: 'Redeem',
      redeeming: 'Redeeming…',
      redeemPrompt: 'Get a code to redeem this offer in person.',
      yourCode: 'Your redeem code',
      showAtCounter: 'Show this code or QR at the counter to redeem.',
      alreadyUsed: 'This offer was already redeemed and marked as used.',
      copyCode: 'Share code',
      downloadQr: 'Download QR',
      downloadingQr: 'Preparing…',
      downloadQrSuccess: 'QR image ready to save or share.',
      downloadQrFailed: 'Could not prepare the QR image.',
      codeCopied: 'Code copied',
      redeemSuccess: 'Redeem code ready — show it at the counter.',
      redeemFailed: 'Could not redeem this offer.',
    },
    follow: '+ Follow',
    following: 'Following',
    noDetails: 'No additional details available.',
    noLocation: 'No location/branch',
    noExpiration: 'No expiration',
    moreBranches: '+ {{count}} more',
    followedBadge: 'From a business you follow',
    nearYouBadge: 'Near you',
    sections: {
      expiringSoon: {
        title: 'Expiring soon',
        subtitle: "Act on these before they're gone",
      },
      followed: {
        title: 'From businesses you follow',
        subtitle: 'You opted in — these come first',
      },
      nearYou: {
        title: 'Featured near you',
        subtitle: 'Offers from businesses in your area',
      },
      moreOffers: {
        title: 'More offers',
        subtitle: 'Other deals you might like',
      },
    },
  },
  offerForm: {
    validation: {
      businessRequired: 'Select a business before creating an offer.',
    },
    error: 'Something went wrong. Please try again.',
  },
  preHomeOnboarding: {
    skip: 'Skip',
    step1: {
      kicker: 'STEP 1 · HOME BASE',
      titleRegular: 'Where do you',
      titleItalic: 'spend most of your time?',
      description: "We'll center your feed, your offers, and your search around this city. You can change it anytime from Settings.",
      useLocation: 'Use my current location',
      gpsDetected: 'GPS-detected',
      searchPlaceholder: 'Or search any city',
      popularCities: 'POPULAR CITIES',
    },
    step2: {
      kicker: 'STEP 2 · YOUR TASTE TRIBE',
      titleRegular: 'Six curators',
      titleItalic: 'who match your blend.',
      description: 'Picked because their lists align with what you said you care about. Follow at least 3 to make Home actually useful on day one.',
      cta: 'Open my Home feed',
      loading: 'Loading curators...',
      loadError: 'Could not load your taste tribe right now.',
      empty: 'No curators found yet. Try again in a bit.',
      roleFallback: 'Curator',
      bioFallback: 'Photographable interiors, design-forward shops.',
    },
  },
  listForm: {
    newListTitle: 'New list',
    editListTitle: 'Edit list',
    shareSettingsTitle: 'Share settings',
    saveDraft: 'Save draft',
    draft: 'Draft',
    publishList: 'Publish list',
    updateList: 'Update list',
    cancel: 'Cancel',
    back: 'Back',
    next: 'Next',
    continue: 'Continue',
    heroTitle: 'Write your list',
    heroSubtitle: 'Give it a name, some vibe, and the spots that earned their place.',
    editHeroTitle: 'Update your list',
    editHeroSubtitle: 'Change the details, picks, and sharing settings for this list.',
    shareHeroTitle: 'How should this be shared?',
    shareHeroSubtitle: "You can change this anytime from the list's settings.",
    fields: {
      listName: 'List name',
      category: 'Category',
      location: 'Location',
      introNote: 'Intro note',
      introNoteHint: 'Optional · 1–2 sentences',
      yourPicks: 'Your picks',
      othersCategory: 'Specify other category',
      addFromPicks: 'Add from your picks',
      addFromPicksHint: "Reuse picks you've saved before",
      createNewPick: 'Or create a new pick',
      permissions: 'Permissions',
      allowComments: 'Allow comments',
      allowCommentsHint: 'Others can react and leave thoughts on your picks',
      allowShare: 'Allow sharing',
      allowShareHint: 'Others can share a link to your list',
      selectedUsers: 'Selected users',
    },
    placeholders: {
      listName: 'e.g. Vegas coffee shops worth the drive',
      location: 'Type a city or neighborhood',
      introNote: 'Add a short intro for your list...',
      othersCategory: 'e.g. Hidden gems',
      searchPicks: 'Search your picks by name, tags, or notes…',
      searchUsers: 'Search for users...',
    },
    picksStatus: '{{count}} added · 1+ required',
    validation: {
      nameRequired: 'Please enter a list name',
      categoryRequired: 'Please select a category',
      othersNameRequired: "Please specify the 'Others' category name",
      notesRequired: 'Please add an intro note',
      picksRequired: 'Please add at least one pick',
    },
    discardTitle: 'Discard list?',
    discardMessage: 'You have unsaved changes. Are you sure you want to leave?',
    discardConfirm: 'Discard',
    draftSaved: 'Draft saved',
    draftError: 'Could not save draft',
    publishSuccess: 'List published!',
    publishError: 'Could not publish list',
    updateSuccess: 'List updated!',
    updateError: 'Could not update list',
    loadError: 'Could not load this list. Please try again.',
    linkPicks: {
      loading: 'Loading your picks…',
      empty: 'You have no saved picks yet. Create one below.',
      allLinked: 'All matching picks are already on this list.',
      add: 'Add',
    },
    visibility: {
      public: {
        label: 'Public',
        description: 'Visible to everyone on the platform',
      },
      friends: {
        label: 'Friends only',
        description: 'Only your followers can see this list',
      },
      private: {
        label: 'Private',
        description: 'Only you can see this list',
      },
      specific: {
        label: 'Specific people',
        description: 'Choose specific people who can view',
      },
    },
  },
  listDetail: {
    loading: 'Loading list…',
    error: 'Could not load this list',
    retry: 'Try again',
    lastPickAdded: 'Last pick added {{time}}',
    storyHint: "Tap {{name}}'s avatar to flip through every photo from this list.",
    openInMaps: 'Open in Maps',
    openAllInMap: 'Open all in map',
    navigateWithGoogleMaps: 'Navigate with Google Maps',
    navigateWithAppleMaps: 'Navigate with Apple Maps',
    navigateWithWaze: 'Navigate with Waze',
    mapAppUnavailable: '{{appName}} is not available on this device.',
    mapOpenFailed: 'Could not open {{appName}}.',
    saveList: 'Save list',
    savedList: 'Saved',
    like: 'Like',
    liked: 'Liked',
    comment: 'Comment',
    comments: 'Comments · {{count}}',
    pickLabel: 'Pick {{number}}',
    pickEyebrow: 'Pick',
    picksSection: 'Picks',
    picksSectionSubtitle: 'Hand-picked places in this list',
    noPicks: 'No picks in this list yet.',
    reactions: {
      idGo: "I'd go",
      notForMe: 'Not for me',
      save: 'Save',
    },
    commentPlaceholder: 'Add a comment…',
    replyPlaceholder: 'Write a reply…',
    replyTo: 'Replying to {{name}}',
    reply: 'Reply',
    cancelReply: 'Cancel',
    postComment: 'Post',
    noComments: 'No comments yet. Start the conversation.',
    searchingMentions: 'Searching…',
    report: 'Report',
    block: 'Block',
    share: 'Share',
    viewReplies: 'View {{count}} replies',
    hideReplies: 'Hide replies',
  },
  smartPick: {
    welcomeEyebrow: 'SmartPicks',
    welcomeTitle: 'Welcome.',
    welcomeTitleAccent: 'Where to?',
    welcomeDetail:
      "We'll match you against your lists, your personality, your friends, and the community. No guesswork.",
    occasionQuestion: "What's the occasion?",
    timeQuestion: 'What time of day?',
    engineQuestion:
      'Want smarter results? Use personality. Want fast picks? Choose a vibe.',
    modePersonality: 'Personality',
    modeVibes: 'Vibes',
    linkProfileLabel: 'Link profile',
    linkProfileHelper:
      "Add anyone you're going with. We'll pick something that works for all of you.",
    linkProfileSearchPlaceholder: 'Search for users...',
    previousPicks: 'Previous picks',
    previousPicksCount: '{{count}} saved',
    skip: 'Skip',
    getRecommendations: 'Get Recommendations',
    forYouEyebrow: 'For you · just now',
    outsideUsualEyebrow: 'Outside your usual',
    yourPick: 'Your pick',
    differentVibe: 'Different vibe',
    orTheseStandard: 'Or these',
    orTheseStretch: 'Or these stretches',
    whyThisMatch: 'Why this match',
    whyThisStretches: 'Why this stretches you',
    curatedBy: 'Curated by {{name}}',
    takeMeThere: 'Take me there',
    takeChance: 'Take a chance',
    noDirections: "We don't have directions for this pick yet.",
    noResultsTitle: 'No recommendations found',
    noResultsDetail: 'Try a different occasion, time, or matching engine.',
    tryAgain: 'Try Again',
    aroundTheWeb: 'Also from around the web',
    enableLocation: 'Enable location access to get nearby suggestions from around the web.',
    saveIt: 'Save it',
    historyTitle: 'Previous picks',
    historyEmptyTitle: 'No previous picks yet',
    historyEmptyDetail: 'Your Smart Pick history will show up here.',
    historyLoadError: 'Could not load your picks',
    sessionLoadError: 'Could not load this Smart Pick session',
  },
  spotlight: {
    loading: 'Loading Spotlight...',
    empty: 'No Spotlight this week',
    emptyDescription: 'Check back soon for a new edition',
    error: 'Could not load Spotlight',
    retry: 'Try again',
    hero: {
      kicker: 'SPOTLIGHT · {{dateRange}}',
      title: 'This week in',
      titleAccent: '{{city}}.',
      subtitle: 'Fresh local picks, lists, and people worth following. Updated every Monday.',
    },
    sections: {
      picks: {
        number: '01',
        title: 'Picks & places worth a detour.',
        empty: 'No picks to spotlight yet',
        emptyDescription: 'Check back soon for standout picks from curators near you',
        saveLabel: 'Save {{title}}',
        loadingLabel: 'Loading pick details',
        carouselLabel: '{{count}} picks and places this week',
      },
      lists: {
        number: '02',
        title: 'Lists we loved.',
        empty: 'No lists to spotlight yet',
        emptyDescription: 'Check back soon for standout lists from curators near you',
        savesCount_one: '{{count}} save',
        savesCount_other: '{{count}} saves',
        saveLabel: 'Save list: {{title}}',
        carouselLabel: '{{count}} more lists this week',
      },
      curators: {
        number: '03',
        title: 'Curator of the week.',
        watchLabel: 'CURATORS TO WATCH',
        empty: 'No curators to spotlight yet',
        emptyDescription: 'Check back soon for standout curators near you',
        listsCount_one: '{{count}} list',
        listsCount_other: '{{count}} lists',
        carouselLabel: '{{count}} curators to watch',
      },
      collections: {
        number: '04',
        title: "This week's collection.",
        kicker: 'COLLECTION',
        empty: 'No collection to show yet',
        emptyDescription: 'Check back soon for a new themed collection',
      },
      businesses: {
        number: '05',
        title: 'Local businesses worth knowing.',
        empty: 'No businesses to spotlight yet',
        emptyDescription: 'Check back soon for standout businesses near you',
        carouselLabel: '{{count}} local businesses this week',
      },
      sponsored: {
        number: '06',
        title: 'Sponsored.',
        tag: 'SPONSORED',
      },
    },
    pickedBy: 'Picked by {{name}}',
    featuredInLists_one: "Featured in {{count}} curator's list",
    featuredInLists_other: "Featured in {{count}} curators' lists",
    inLists_one: "In {{count}} curator's list",
    inLists_other: "In {{count}} curators' lists",
    swipeCue: 'Swipe',
    fallbackChip: 'FROM {{region}}',
    curatorOfWeek: {
      badge: 'OF THE WEEK',
      viewLists: 'View lists',
    },
    follow: '+ Follow',
    following: 'Following',
    verified: 'Verified',
    collectionFooterCta: 'Explore this collection',
    collectionListMeta: '{{name}} · {{count}} picks',
  },
  notifications: {
    title: 'Notifications',
    markAllAsRead: 'Mark all as read',
    empty: "Quiet today. We'll let you know when something happens.",
    emptyFiltered: 'No notifications in this category yet.',
    loadingError: "Couldn't load notifications. Pull to try again.",
    sections: {
      new: 'NEW',
      earlier: 'EARLIER',
    },
    chips: {
      all: 'All',
      mentions: '@ Mentions',
      saves: 'Saves',
      newLists: 'New lists',
      offers: 'Offers',
    },
    actions: {
      savedYourList: 'saved your list',
      likedYourList: 'liked your list',
      startedFollowing: 'started following you',
      spotlightFeatured: 'your list was featured in Spotlight',
      businessViewed: 'viewed a business you follow',
      fallback: 'sent you a notification',
      someone: 'Someone',
      aList: 'a list',
    },
  },
  businessHome: {
    title: 'Business Home',
    managingAs: 'Managing as {{name}} · {{role}}',
    comingSoonTitle: 'Coming soon',
    comingSoonMessage: 'This feature is not available yet.',
    switcher: {
      yourBusinesses: 'Your businesses',
      locations: 'Locations',
      locationCount_one: '{{count}} location · {{role}}',
      locationCount_other: '{{count}} locations · {{role}}',
      loading: 'Loading your businesses…',
      error: 'Could not load businesses',
      retry: 'Try again',
      empty: 'No businesses linked to this account yet.',
      noLocations: 'No locations for this business yet.',
    },
    dateRangePicker: {
      title: 'Select period',
      apply: 'Apply',
      cancel: 'Cancel',
    },
    shortcuts: {
      promote: 'Promote',
      campaigns: 'Campaigns',
      analytics: 'Explore analytics',
    },
    stats: {
      views: 'VIEWS',
      saves: 'SAVES',
      redeemed: 'REDEEMED ✓',
      lists: 'LISTS',
    },
    insight: {
      paidBody:
        'Views rose 18% — mostly because two new Lists added you. Those visitors save at a lower rate than your usual audience, which is why saves rose only 6%: your discovery is improving faster than your conversion.',
      paidSource: 'Your data ✦ Automatic interpretation · Business Insights',
      freeBody:
        'Views ↑18% · Saves ↑6% this period. Every number here is yours, free, always.',
      freeSource:
        '✦ Want to know why these moved differently? Business Insights can explain the change and recommend what to do next.',
      seeInsights: 'See Business Insights',
    },
    sections: {
      thisWeek: 'This week',
      alerts: 'Alerts',
      nextActions: 'Your next actions',
      insightsTools: 'Business Insights tools',
      howPeopleFindYou: 'How people find you',
      campaignResults: 'Campaign results',
      locations: 'Across your locations',
      exploreInsights: 'Explore insights',
      monthlyReport: 'Monthly report',
      promoteTools: 'Promote · pay per campaign, no membership',
      businessInsights: 'Business Insights',
    },
    brief: {
      kicker: 'Copilot · {{period}}',
      body:
        'Your Saturday service plan is holding — mentions down 3 → 1 — and the cardamom bun test confirmed real demand. Late-hour searches keep rising.',
      nextLabel: 'Best next move:',
      nextMove: 'Promote the bun while the Saturday fix finishes.',
      assignRow: 'Assign to Sarah · Manager ▾ · due this week ▾',
      assignedOk: 'Assigned to Sarah ✓',
    },
    alert: {
      saturday: 'Saturday',
      sunday: 'Sunday',
    },
    action: {
      priority: 'Priority',
      effort: 'Effort',
      confidence: 'Confidence',
      recommendedStep: 'Recommended next step',
      measure: 'Measure',
    },
    buttons: {
      createOffer: 'Create Offer',
      redeemTool: 'Redeem tool',
      supportingData: 'Supporting data',
      assign: 'Assign',
      confirmHours: 'Confirm Hours',
      assignToSarah: 'Assign to Sarah',
      dismiss: 'Dismiss',
      confirm: 'Confirm',
      changeHours: 'Change hours',
      viewPlan: 'View plan',
      startActionPlan: 'Start action plan',
      promoteOpportunity: 'Promote this opportunity',
      explain: 'Explain',
      reachMore: 'Reach more people like this',
      runAgain: 'Run it again',
      editRelaunch: 'Edit & relaunch',
      receipt: 'Receipt',
      submitSpotlight: 'Submit Spotlight bid',
      fixNow: 'Fix now',
      compareLocations: 'Compare locations',
      addLocation: 'Add a location',
      createMorningOffer: 'Create Morning Offer',
    },
    tools: {
      copilot: 'Copilot',
      improveProfile: 'Improve profile',
      actionPlans: 'Action plans',
      alerts: 'Alerts',
      repeatCampaign: 'Repeat campaign',
    },
    discovery: {
      search: 'Search',
      lists: 'Lists',
      picks: 'Picks',
      discover: 'Discover',
      seeDetails: 'See details',
    },
    campaign: {
      spend: 'SPEND',
      redeemed: 'REDEEMED ✓',
      sales: 'TRACKED SALES',
      roas: 'TRACKED ROAS',
      viewSnapshot: 'View insight snapshot',
      campaignSetup: 'Campaign setup',
      unlockInsights: 'Unlock Business Insights to compare this with previous campaigns and get your next recommended action.',
      paidNote: 'Strongest with morning-focused customers. Next recommended action is in your plan above.',
      compare: '⇄ Compare campaigns',
    },
    runAnother: {
      title: 'Run another campaign',
      body: 'Each purchase includes its own Campaign Insight Snapshot and limited Campaign Assistant — scoped to that campaign only.',
    },
    profileHealth: {
      title: 'Profile health · {{score}}/100',
      freeBadge: 'FREE',
    },
    locations: {
      viewing: 'viewing',
      note: 'Each location keeps its own public profile. One account and team can manage all locations — Business Insights membership is billed per subscribed location on the web.',
      paidNote: '✦ "Slow service" is emerging at Scottsdale only — the playbook from your Roosevelt Row fix applies.',
    },
    explore: {
      customers: 'Customers',
      demand: 'Demand',
      performance: 'Performance',
      whatCustomersSay: 'What customers say',
      customersSource:
        '32 public pick notes · 1,400 savers · emerging: cardamom bun ▲, weekend speed ⚠ · fit & benchmarks live in Performance',
      viewAllCustomers: 'View all customer insights →',
      whoTheyAre: 'Who they are',
      personalitySource:
        '1,400 savers with taste profiles · top areas: Roosevelt Row 34%, Arcadia 22% · vs. café average: Curators over-represented 1.6×',
      personalityPaidNote:
        '✦ Curator-heavy audiences respond best to specificity — named products, named seats, named hours. Broad promotions underperform for you.',
      whatMakesDifferent: 'What makes them different',
      diffSource:
        'Based on the 61% of your savers with taste profiles · vs. Phoenix café average',
      diffPaidNote: '✦ Spend on the product, not the presentation — this audience rewards substance.',
      demandSource: 'Search impressions Jul 1–28 — impressions, not unique people',
      viewDemandReport: 'View full demand report →',
      demandOpportunity:
        'Local activity Demand for late-night study cafés is increasing near you.',
      whenFindYou: 'When people find you',
      strongest: 'Strongest interest',
      quietest: 'Quietest',
      performanceSource: 'Category benchmark Compared with 184 Phoenix cafés',
      viewPerformanceReport: 'View full performance report →',
    },
    monthlyReport: {
      rowTitle: 'Monthly report',
      ready: 'July report ready →',
    },
    upsell: {
      membership: 'Membership',
      body: 'All your analytics are free, always. Membership turns them into an intelligent business assistant — the why, what matters, and what to do next.',
      paymentNote:
        'Two ways to pay, ever: campaigns are pay-per-use · Insights is one monthly membership. That\'s the whole structure.',
      includes: {
        copilot: 'Business Copilot — ask anything about your data',
        brief: 'Weekly brief with your best next move',
        alerts: 'Smart alerts & automatic opportunity detection',
        interpretation: 'Automatic interpretation & cross-data analysis',
        actionPlans: 'Action plans & AI profile optimization',
        campaignIntel: 'Campaign intelligence & monthly report',
      },
      continueOnWeb: 'Continue on web',
      openWebError: 'Could not open Business Insights on the web.',
    },
    footerNote:
      'Public picks and list adds always show who made them. Saves, favorites and views are private — totals only. Paying for a campaign includes campaign data and limited campaign AI; it does not unlock ongoing Copilot. Reaching a data threshold does not unlock paid features. Searches are impressions, not unique people; tracked sales are counter-verified. Nothing publishes without your approval.',
    sheets: {
      copilot: {
        title: '✦ Business Copilot',
        sampleQuestion: 'Why did saves dip last week?',
        sampleAnswer:
          'One cause, not a trend: you left "Best WiFi cafés" (46 businesses) on Jul 22. Saves from that list went to zero while every other source grew.',
        cite1: '📉 List placements · Jul 15–28',
        cite2: '📊 Observed',
        turnIntoPlan: 'Turn into action plan',
        draftNote: 'Draft a note to the curator',
        suggest1: 'What should I focus on this week?',
        suggest2: 'What are customers starting to mention?',
        suggest3: 'Which hours should I test?',
      },
      profile: {
        title: '✎ Improve profile',
        healthTitle: 'Profile health',
        healthBody: 'Strong basics. Two gaps are costing you discovery — both fixable today.',
        hoursTitle: 'Confirm weekend hours',
        hoursBody:
          'Saturday searches can\'t surface you confidently while hours are unconfirmed — and Saturday is your most-mentioned day.',
        descriptionTitle: 'Refresh your description',
        descriptionBody:
          'Your bio doesn\'t mention the words customers actually search for. AI draft:',
        draft:
          '"A quiet Roosevelt Row coffee bar for slow mornings and long study sessions — house cardamom buns at 7, rotating guest espresso, window seats worth waiting for."',
        useDraft: 'Use draft',
      },
      plans: {
        title: '⚑ Action plans',
        activeTitle: 'Saturday staffing review',
        activeBody:
          'Assigned to Marcus · Store Manager · Goal: Zero new "slow" mentions · Baseline: 3 mentions / 30 days · So far: 1 mention · trending down',
        logNumbers: 'Log this week\'s numbers',
        completedTitle: 'Cardamom bun +20%',
        completedBody:
          'Owner of task: Sarah · Manager · Result: sell-out moved 8:40am → 10:15am, units +19%, zero waste days.',
      },
      alerts: {
        title: '🔔 Alerts',
        attentionTitle: '"Weekend speed" mentioned again',
        attentionBody:
          'What changed: a 3rd pick note in 20 days mentioned slow Saturday service. Why it matters: recurring concerns spread into list notes and cost placements.',
        opportunityTitle: 'Late-night demand increased 31%',
        opportunityBody:
          'Thursday–Saturday shows the strongest growth, and your study-focused audience already fits it.',
      },
      report: {
        title: '📊 July report',
        oneLineTitle: 'The month in one line',
        oneLineBody:
          'Your best save month yet — driven by the bun campaign and the study-list placement — with one recurring concern (Saturday service) now under active repair.',
        improvedTitle: 'Improved ↗',
        saves: 'Saves 2.1K · +34%',
        redemptions: 'Verified redemptions 96 · first campaign',
      },
      compare: {
        title: '⇄ Campaign comparison',
        headline: 'Your two campaigns, side by side',
        campaignA: 'Bun + cortado\nmorning · $50 · 96 ✓',
        campaignB: 'Afternoon special\nMay · $40 · 22 ✓',
        meaningTitle: 'What the difference means',
        meaningBody:
          'Morning-focused customers respond ~3.7× better for you. The audiences barely overlap: the morning campaign converted your study-list savers; the afternoon one reached mostly new viewers who saved but didn\'t redeem.',
      },
    },
  },
  redeemOffer: {
    title: 'Redeem tool',
    codeLabel: 'Redeem code',
    codePlaceholder: 'Enter code',
    codeRequired: 'Enter a redeem code to validate.',
    validate: 'Validate',
    validating: 'Validating…',
    scanQr: 'Scan QR',
    scanTitle: 'Scan redeem QR',
    scanHint: 'Point the camera at the customer’s redeem QR code.',
    scanPermissionNeeded: 'Camera permission is required to scan QR codes.',
    scanPermissionDenied: 'Camera access was denied. You can still enter the code manually.',
    scanClose: 'Close scanner',
    uploadQr: 'Upload QR',
    uploadingQr: 'Reading…',
    uploadPermissionDenied:
      'Photo library access was denied. You can still enter the code manually or scan with the camera.',
    uploadNoQrFound: 'No QR code found in that image.',
    uploadFailed: 'Could not read a QR code from that image.',
    markUsed: 'Mark as used',
    markingUsed: 'Marking…',
    offerLabel: 'Offer',
    customerLabel: 'Customer',
    statusUnused: 'Unused — ready to mark as used',
    statusUsed: 'Already used',
    usedAt: 'Used {{time}}',
    validateFailed: 'Could not validate this code.',
    useSuccess: 'Code marked as used.',
    useFailed: 'Could not mark this code as used.',
    unknownCustomer: 'Unknown customer',
    historyTitle: 'Redeemed codes',
    historyEmpty: 'No codes have been marked as used yet.',
    historyError: 'Could not load redeemed codes.',
    historyRetry: 'Try again',
    historyLoadMore: 'Load more',
    historyLoading: 'Loading…',
  },
  convertToBusiness: {
    title: 'Convert to business',
    helper:
      'Fill in your business details. A valid business contact email is required.',
    submit: 'Convert account',
    submitting: 'Converting…',
    saveSuccess: 'Your account is now a business account.',
    saveFailed: 'Could not convert your account. Please try again.',
    addTitle: 'Add another business',
    addHelper:
      'Create another business under your account. It will become your primary business for editing and Business Home.',
    addSubmit: 'Add business',
    adding: 'Adding…',
    addSuccess: 'Business added successfully.',
    addFailed: 'Could not add this business. Please try again.',
  },
  editProfile: {
    business: {
      section: 'Business information',
      name: 'Business name',
      namePlaceholder: 'Bean There Coffee Shop',
      type: 'Business type',
      typePlaceholder: 'Select type',
      bio: 'Bio',
      bioPlaceholder: 'Write about your business here',
      contactEmail: 'Contact email',
      contactEmailPlaceholder: 'contact@example.com',
      phone: 'Phone number',
      phonePlaceholder: '+1 (555) 123-4567',
      website: 'Website',
      websitePlaceholder: 'https://example.com',
      logo: 'Business logo',
      logoHelper: 'PNG or JPG up to 5MB',
      hoursSection: 'Opening hours',
      hoursHelper: 'Optional. Set open and close times for each day.',
      hoursOpen: 'Open',
      hoursClose: 'Close',
      hoursClear: 'Clear',
      hoursUnset: 'Not set',
      hoursIncomplete: 'Set both open and close times for each day you configure.',
      hoursInvalid: 'Use a valid time for open and close.',
      hoursOrder: 'Open time must be before close time.',
      editBranchHours: 'Edit hours',
      editBranchHoursTitle: 'Hours · {{name}}',
      weekdays: {
        monday: 'Monday',
        tuesday: 'Tuesday',
        wednesday: 'Wednesday',
        thursday: 'Thursday',
        friday: 'Friday',
        saturday: 'Saturday',
        sunday: 'Sunday',
      },
      branchesSection: 'Branch locations',
      branchesHelper: 'Manage all your business locations',
      noBranches: 'No branch locations added yet.',
      addBranch: 'Add branch',
      addingBranch: 'Adding…',
      branchName: 'Branch name',
      branchNamePlaceholder: 'Downtown branch',
      branchNameRequired: 'Branch name is required',
      branchAddress: 'Address',
      branchAddressPlaceholder: 'Search Google Maps address',
      branchLocationRequired: 'Select an address from search results',
      removeBranch: 'Remove branch',
      removeBranchMessage:
        'Are you sure you want to remove this branch? This action cannot be undone.',
      nameRequired: 'Business name is required',
      emailRequired: 'Contact email is required',
      phoneRequired: 'Phone number is required',
      emailInvalid: 'Enter a valid email address',
      websiteInvalid: 'Enter a valid URL (e.g. https://example.com)',
      saveFailed: 'Could not update business details.',
      loadFailed: 'Could not load business details.',
      switchBusiness: 'Switch business',
      editingBusiness: 'Editing',
      primaryBadge: 'Primary',
      noOwnedBusinesses: 'No owned businesses found.',
      discardSwitchTitle: 'Discard changes?',
      discardSwitchMessage:
        'You have unsaved business or profile changes. Switch business and discard them?',
      discardSwitchConfirm: 'Discard & switch',
      switchFailed: 'Could not switch business. Please try again.',
    },
  },
  claimBusiness: {
    start: {
      title: 'Claim a business',
      findExisting: 'Find an existing business',
      findExistingSub: 'Search LocalNotes businesses that are already claimed',
      claimFromPick: 'Claim from a pick',
      claimFromPickSub: 'Claim a place someone already added as a pick',
      yourClaims: 'Your claims',
      noClaims: 'You have not submitted any claims yet.',
      loadError: 'Could not load your claims.',
      retry: 'Try again',
      rejectedNote: 'Reason: {{note}}',
    },
    search: {
      title: 'Choose a business',
      businessesTab: 'Businesses',
      picksTab: 'Picks',
      placeholder: 'Search by name…',
      empty: 'No claimable results found.',
      loadError: 'Could not load results.',
      retry: 'Try again',
      pickUnverified: 'Unverified place',
      pickExisting: 'Listed business',
    },
    form: {
      title: 'Claim this business',
      claiming: 'Claiming {{name}}',
      untitledBusiness: 'this business',
      heroLead: 'Is this',
      heroEm: 'your business?',
      heroBody:
        "Verify ownership and you'll get the keys to manage {{name}} on LocalNotes — reply to reviews, post offers, see who's saving you, and update your hours.",
      pickPath: 'Pick a verification path',
      fastest: 'Fastest',
      pathEmailTitle: 'Email at your business domain',
      pathEmailBody:
        "We'll send a 6-digit code to {{email}}. Instant verification.",
      pathEmailUnavailable:
        'This listing has no work email on file for verification.',
      pathEmailTiming: 'Under 2 minutes',
      pathPhoneTitle: 'Phone call to your listed number',
      pathPhoneBody:
        "We'll call {{phone}} and read a code aloud. Pick up and enter it.",
      pathPhoneUnavailable:
        'This listing has no phone number on file for verification.',
      pathPhoneTiming: 'Under 5 minutes',
      pathDocumentTitle: 'Upload a document',
      pathDocumentBody:
        'Business license, utility bill, or lease showing the business name and address.',
      pathDocumentTiming: '24–72 hour review',
      pathUnavailable: 'Not available for this listing',
      terms:
        "By claiming, you agree to our Business Terms and confirm you're authorized to manage this listing.",
      emailVerifyTitle: 'Email verification',
      phoneVerifyTitle: 'Phone verification',
      documentTitle: 'Upload proof',
      stepCodeSent: 'Step 1 of 2 · Code sent',
      stepCodeCalled: 'Step 1 of 2 · Calling now',
      enterCode: 'Enter the 6-digit code',
      codeSentTo: 'We sent a code to {{email}}. It expires in 10 minutes.',
      codeCalledTo:
        "We're calling {{phone}} and will read a code aloud. It expires in 10 minutes.",
      didntGetIt: "Didn't get it?",
      resendCode: 'Resend code',
      resendCall: 'Call again',
      tryDifferentPath: 'Try a different path',
      verifyCode: 'Verify code',
      verifying: 'Verifying…',
      otpInvalid: 'That code is invalid or expired. Please try again.',
      timelineEmailSent: 'Email sent',
      timelineCallPlaced: 'Call placed',
      timelineJustNow: 'Just now · {{time}}',
      timelineVerify: 'Verify the code',
      timelineInProgress: 'In progress',
      timelineReview: 'Admin review',
      timelineAfterVerify: 'After verification',
      workEmail: 'Work email',
      workEmailHint: 'Use a company email — not Gmail, Yahoo, or Hotmail',
      workEmailInvalid: 'Please provide a work email address.',
      phone: 'Phone number',
      phoneRequired: 'Phone number is required',
      proof: 'Proof of ownership',
      proofHint: 'JPEG, PNG, or PDF · max 10 MB',
      proofRequired: 'Proof of ownership is required',
      proofPickImage: 'Choose photo',
      proofPickDocument: 'Choose PDF',
      proofClear: 'Remove file',
      proposedName: 'Business name',
      proposedNameRequired: 'Business name is required',
      proposedType: 'Business type',
      proposedTypePlaceholder: 'Select type',
      proposedWebsite: 'Website',
      proposedLocation: 'Location',
      submit: 'Submit claim',
      submitting: 'Submitting…',
      submitError: 'Could not submit your claim. Please try again.',
    },
    submitted: {
      title: 'Claim submitted',
      body: 'We will review your claim and notify you when it is approved or rejected.',
      nextTitle: 'What happens next',
      nextReview: 'Our team reviews your claim',
      nextNotify: 'You get notified of the decision',
      nextManage: 'You manage your business listing',
      viewClaims: 'View my claims',
      done: 'Done',
    },
    status: {
      pending: 'Pending',
      approved: 'Approved',
      rejected: 'Rejected',
    },
    detail: {
      title: 'Claim details',
      contact: 'Contact',
      existingBusiness: 'Business',
      proposedBusiness: 'Proposed business',
      proof: 'Proof of ownership',
      rejection: 'Rejection reason',
      workEmail: 'Work email',
      phone: 'Phone',
      otpVerified: 'OTP verified',
      name: 'Name',
      type: 'Type',
      website: 'Website',
      location: 'Location',
      empty: '—',
      viewProof: 'View proof image',
      openPdf: 'Open PDF',
      noDocument: 'No document uploaded — verified via {{method}}.',
      submitted: 'Submitted {{date}}',
      sourceExisting: 'Existing business',
      sourcePickExisting: 'Pick → existing business',
      sourcePickUnverified: 'Pick → new business',
      methodEmail: 'Email OTP',
      methodPhone: 'Phone call OTP',
      methodDocument: 'Document upload',
    },
    entry: {
      isYourBusiness: 'Is this your business?',
      claimThisBusiness: 'Claim this business',
    },
  },
  support: {
    title: 'Support',
    intro:
      'Questions, feedback, or account help — reach LocalNotes using the contact details below.',
    contactSection: 'Contact us',
    legalSection: 'Legal',
    phone: 'Phone',
    address: 'Address',
    email: 'Email',
    privacyPolicy: 'Privacy Policy',
    termsOfService: 'Terms of Service',
    communityGuidelines: 'Community Guidelines',
  },
} as const;

export default en;
