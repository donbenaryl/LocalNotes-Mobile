...
interface ThankYouContentProps {
  /** When true, skip outer flex fill (embedded in another scroll/page). */
  embedded?: boolean;
}

export function ThankYouContent({ embedded = false }: ThankYouContentProps) {
  ...
  const body = (
    <>
      ...
      {hasRecipients ? (
        <LocalNotesButton
          label={
            sendMutation.isPending ? t('thankYou.sending') : t('thankYou.send')
          }
          onPress={handleSend}
          variant="brand"
          size="sm"
  ...
  if (embedded) {
    return <View className="gap-4 px-4 pb-8 pt-2">{body}</View>;
  }
